...
class TicTacToe:
    ...
    def move(self):  # ход
        def is_move_valid(x, y): #фактичность шага
        ...
        while not is_coords_valid:
            y, x = input('Введите координаты x и y через пробел: ').split() #Подстроки
            x, y = int(x), int(y)
            is_coords_valid = is_move_valid(x, y)
            if not is_coords_valid: #проверка на координаты
                print('Ошибка, вы ввели неправильные координаты!')
        self.table[x-1][y-1] = self.current_move # изменение знаков на поле
    ...
    def reload(self):  # перезагрузка игры
        print('Началась игра!')
        self.table = [['*', '*', '*'], ['*', '*', '*'], ['*', '*', '*']] #создание игрового поля
    ...
    def start(self):  # начало игры
        while True:
            ...
            if self.winner:  # если победитель есть - завершаем игру
                break
            self.change_current_move() #поменять нынешний ход
    ...
    def get_current_player_name(self):  # получение имени текущего игрока
        return self.player_x if self.current_move == 'x' else self.player_0 #возврат хода
